...
for B0 in Bs:
    ...
    fig, (ax0, ax1, ax2, ax3) = plt.subplots(4,1,sharex=True,figsize=(15,15))
    sel = np.abs(r.y[1])/r.y[1][0] < 1.5
    ax0.set_title(r"$B_0$ = {:.4f} T".format(B0),fontsize="32")
    ...
    ax3.plot(np.zeros_like(r.t),
            ...
            'black', linewidth=.3)
    ax0.set_ylabel(r'$\left<T_{\perp}\right>$')
    ax1.set_ylabel(r'$\left<\rho^2\right>$')
    ax2.set_ylabel(r'$\left<L_z\right>$')
    ax3.set_ylabel(r'$\left<G_{\perp}\right>$')
    ax3.set_xlabel('z/a')
    plt.show()
