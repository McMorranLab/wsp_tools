...
'''
Main arguments:
-filename: defaults to 'cval_{0}_angle_{1}.npz'.format(cval,angle)
...
-angle: defaults to 4.; the half angle of the cone
-cval: defaults to 8.; prop to the potential the cone is held at
'''
...
def init_bcparams(f,X,Y,Z,main_args):
    ...
    angle = 2*np.pi/360 * main_args[1]
    bcparams = (cval,angle)
    return(bcparams)

def setBCs(g,X,Y,Z,bcparams):
    cval,angle = bcparams
    g[np.where(np.arctan2(np.sqrt(Y**2+Z**2),-X)<=angle)] = cval
...
if __name__ == "__main__":
    ...
    parser.add_argument('-angle',default=4.0)
    args = parser.parse_args()
    if args.filename == 'dummy':
        args.filename = 'cval_{0}_angle_{1}'.format(args.cval,args.angle)
    main(args.filename,(args.cval,args.angle),args.numDub,args.numsRuns)
