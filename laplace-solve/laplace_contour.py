...
'''
Main arguments:
-filename: defaults to 'cval_{0}_contour{1}.npz'.format(cval,contour)
...
-contour: defaults to 8.; proportional to the contour held fixed
-cval: defaults to 8.; proportional to the potential the contour is held at
'''
...
def init_bcparams(f,X,Y,Z,main_args):
    Videal = k*a/foc/CE*np.log((X+b+np.sqrt((X+b)**2+Y**2+Z**2))/(X+np.sqrt(X**2+Y**2+Z**2)))
    cval = k*a/foc/CE * main_args[0]
    contour = k*a/foc/CE * main_args[1]
    bcparams = (cval,contour,Videal)
    return(bcparams)

def setBCs(g,X,Y,Z,bcparams):
    cval,contour,Videal = bcparams
    g[np.where(Videal>=contour)] = cval
...
if __name__ == "__main__":
    ...
    ############################################################################
    parser.add_argument('-cval',default=8.0)
    parser.add_argument('-contour',default=8.0)
    args = parser.parse_args()
    if args.filename == 'dummy':
        args.filename = 'cval_{0}_contour_{1}'.format(args.cval,args.contour)
    main(args.filename,(args.cval,args.contour),args.numDub,args.numsRuns)
